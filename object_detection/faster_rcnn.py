...
class PretrainedFasterRCNN():
    def __init__(self):
        ...
        if self.cuda > 0:
            ...
            checkpoint = torch.load(self.load_name, map_location=torch.device('cpu'))
        else:
    ...
    def _initialize(self):
        ...
        # ship to cuda
        if self.cuda > 0:
            self.im_data = self.im_data.to(device)
            self.im_info = self.im_info.to(device)
            self.num_boxes = self.num_boxes.to(device)
            self.gt_boxes = self.gt_boxes.to(device)
    ...
    def detect_object(self, img):  
        ...
        if self.cuda > 0:
            keep_boxes = torch.where(max_conf >= self.conf_thresh, max_conf, torch.tensor(0.0).to(device))
        else:
